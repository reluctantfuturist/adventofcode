...
const day_1 = require("../../../src/day");
class Day1 extends day_1.Day {
    constructor() {
        super(1);
    }
    convertWordsToDigits(line) {
        const digitWords = [
            "one",
            "two",
            "three",
        ...
        for (let i = 0; i < line.length; i++) {
            ...
            if (char >= "0" && char <= "9") {
                ...
                for (let j = i + 1; j <= line.length; j++) {
                    const potentialDigit = getDigitValue(line.substring(i, j).toLowerCase());
                    if (potentialDigit) {
        ...
        for (let i = line.length - 1; i >= 0; i--) {
            ...
            if (char >= "0" && char <= "9") {
                ...
                for (let j = i; j >= 0; j--) {
                    const potentialDigit = getDigitValue(line.substring(j, i + 1).toLowerCase());
                    if (potentialDigit) {
    ...
    sumCalibrationValues(lines) {
        ...
        for (const line of lines) {
            ...
            if (digits) {
                ...
                const calibrationValue = digits.length === 1
    ...
    sumCalibrationValuesWithWords(lines) {
        let sum = 0;
        for (const line of lines) {
